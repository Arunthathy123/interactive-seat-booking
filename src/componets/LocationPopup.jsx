...
const LocationPopup = ({ onSelectLocation, selectedLocation, openPopup }) => {
    ...
    useEffect(() => {
        ...
        locationElements.forEach(({ id, city }) => {
            ...
            if (element) element.addEventListener('click', () => handleLocationClick(city));
        });

        return () => {
            locationElements.forEach(({ id }) => {
                const element = document.getElementById(id);
                if (element) element.removeEventListener('click', handleLocationClick);
            });
        };
